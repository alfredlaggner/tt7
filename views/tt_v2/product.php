	...
	<div id="container">
		...
		<div id="product-image" class="sg-17">
			...
			<form id="product-image-buy" method="post"
			      action="<?= site_url() . 'tt_v2/product_booking1/' . $row->activity_id ?>">
				<a href="<?= site_url() . 'tt_v2/product_booking1/' . $row->activity_id ?>" id="product-image-buy-link">see
					dates and book</a>
			</form>
		</div>
	...
	<? endforeach // $records as $row?>
		...
		<? foreach ($activities_related as $related) : ?>
			...
			<div class="sg-8 collection-product focus-box"><a
					href="<?= site_url() . "tt_v2/product/" . $related->activity_id ?>"><img src="<?= $picture ?>"
			                                                                                 class="preload"
			                                                                                 alt="<?= $related->name ?>"/></a>
				<div class="line"></div>
				<div class="collection-product-description"><? $related->slogan ?><a
						href="<?= site_url() . "tt_v2/product/" . $related->activity_id ?>"><?= $related->name ?></a>
