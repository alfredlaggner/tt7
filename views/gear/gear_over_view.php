					...
					<table id="sort-table">
						...
						<? if (isset($records)) : foreach ($records as $row) : ?>
							<tr>
								...
								<td><a class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="Edit gear"
								       href="<?= site_url() . 'gear/gear_view/' ?><?= $row->gear_id; ?>"> <span
											class="ui-icon ui-icon-wrench"></span> </a>
									<a class="btn_no_text btn ui-state-default ui-corner-all tooltip"
									   title="Edit Pictures"
									   href="<?= site_url() . 'gear_pictures/index/0/' ?><?= $row->gear_id . '/' . $row->code ?>">
										<span class="ui-icon ui-icon-image"></span> </a>

									<a class="btn_no_text btn ui-state-default ui-corner-all tooltip"
									   title="Related Gears"
									   href="<?= site_url() . 'gear_related/index/' ?><?= $row->gear_id ?>"> <span
											class="ui-icon ui-icon-link"></span> </a>


									<a class="btn_no_text btn ui-state-default ui-corner-all tooltip confirmClick"
									   title="Delete gear"
									   href="<?= site_url() . 'gear/delete/' ?><?= $row->gear_id; ?>"> <span
