...
<div style=" padding: 0.4em; padding-left: 1.5em;  padding-right: 1.5em; display: none; " id="do_book<?= $j ?><?= $i ?>"
     title="You chose <?= date('D, M j', strtotime($event['event_date'])) ?>   (Just press ESC to correct)">
	<div id="customer_data">
		...
		<ul>
			...
			<br><br>
			<li><label for="promo_code">Enter Discount Code <a class="tooltip"
			                                                   title="If you have NO discount code leave field blank. <br>If you already paid for this class with a discount website or got a discount from Treks and Tracks, please enter your discount code now. <br><br>Question? Call us at 650-557-4893 Mon-Fri, 9-5 PST or email info@treksandtracks.com"
			                                                   href="#">Help</a> </label>
			</li>
			<li><input type="text" class="text" onKeyDown="displayResult<?= $j; ?><?= $i; ?>()" onKeyUp=
				"xajax_verify_promo_code(xajax.getFormValues('booking<?= $j ?><?= $i ?>'),<?= $j ?><?= $i ?>,<?= $event['event_event_id'] ?>);return false;"
			           size="10" id="promo_code<?= $j ?><?= $i ?>" name="promo_code" value=""></li>
			<div id="customer_data">

				<input type="submit" id="update<?= $j ?><?= $i ?>" name="update<?= $j ?><?= $i ?>" value="CONTINUE"
				       class="submit"/></div>
			</form>
		</ul>
		</p>
